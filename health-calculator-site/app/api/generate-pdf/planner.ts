...
function toMealFood(row: Record<string, unknown>): MealFood {
  return {
    name:            row.meal_name as string,
    calories:        row.total_kcal as number,
    protein_g:       Number(row.protein_g) || 0,
    carbs_g:         Number(row.carbs_g)   || 0,
    fat_g:           Number(row.fat_g)     || 0,
    fibre_g:         Number(row.fibre_g)   || 0,
    serving_grams:   (row.serving_grams as number) || 100,
    serving_unit:    'g',
    scaled_grams:    (row.serving_grams as number) || 100,
    serving_display: (row.serving_display as string) || `${(row.serving_grams as number) || 100}g`,
  }
}
...
async function fetchSlot(
  ...
  return (data as Record<string, unknown>[]).map(toMealFood)
}
...
export async function buildMealPlan(intakeData: Record<string, unknown>, goal: string): Promise<MealPlan> {
  const {
    name,
    diet_type    = 'veg',
    allergens    = [],
  ...
  } = intakeData as Record<string, any>
  ...
  const tdee = (intakeData.tdee as number) ?? Math.round(bmr * (actMultiplier[activity_level] ?? 1.375))

  const targetWeightKg: number | null = (intakeData.target_weight as number) ?? (intakeData.target_weight_kg as number) ?? null
