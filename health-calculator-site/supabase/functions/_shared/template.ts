// Shared chunk renderers — 9-page PDF split into 3 Browserless requests (~3 pages each)
// to stay under the 150 MB worker limit. Types are inlined to avoid cross-directory imports.
        ...
        <div style="display:grid;grid-template-columns:1fr auto;gap:16px;align-items:center;">
          <div>
            ...
            <div style="display:flex;align-items:center;gap:9px;margin-bottom:7px;">
              <span style="font-size:10px;color:#F1F5F2;width:130px;flex-shrink:0;">${nu.name}</span>
              <div style="flex:1;height:8px;background:#1C1F1D;border-radius:99px;overflow:hidden;">
...
function page3(plan: MealPlan, intakeData: any): string {
  ...
  durations.forEach((dur, i) => {
    ...
    s += `<text x="${PAD.l + i * 68 + 13}" y="${VH + 4}" font-size="8" fill="rgba(255,255,255,.45)" font-family="DM Sans,sans-serif">${dur} min</text>`
  })
    ...
    <div style="position:relative;z-index:1;flex:1;padding:11px 28px;display:flex;flex-direction:column;gap:10px;">
      ...
      <div style="background:#131614;border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:13px;position:relative;overflow:hidden;flex-shrink:0;">
        ...
        <div style="display:flex;gap:14px;">
          <div style="display:flex;align-items:center;gap:5px;font-size:9.5px;color:#7A8C82;"><div style="width:8px;height:8px;border-radius:2px;background:#22C55E;"></div>Diet contribution</div>
          <div style="display:flex;align-items:center;gap:5px;font-size:9.5px;color:#7A8C82;"><div style="width:8px;height:8px;border-radius:2px;background:#F97316;"></div>Exercise contribution</div>
...
function mealLibraryPage(
  ...
  for (const day of plan.days) {
    for (const food of day[mealKey]) {
      if (!seen.has(food.name)) { seen.add(food.name); allFoods.push(food) }
    }
  }
  ...
  <div class="page">
    ...
    <div style="position:relative;z-index:1;flex-shrink:0;margin:3px 26px 0;background:#131614;border:1px solid rgba(255,255,255,.08);border-radius:11px;padding:7px 12px;display:grid;grid-template-columns:1fr 1fr;gap:4px 14px;overflow:hidden;">
...
function page9(plan: MealPlan): string {
  ...
  <div class="page">
    ...
    <div style="position:relative;z-index:1;flex:1;padding:8px 26px;overflow:hidden;">
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr style="background:#1C1F1D;">
        ...
        <tbody>
          ...
          <tr style="border-bottom:1px solid rgba(255,255,255,.04);">
            ...
            <td style="padding:6px 8px;vertical-align:top;">${day.breakfast.map(f => `<div style="font-size:8.5px;color:#F1F5F2;line-height:1.4;">${f.name}</div><div style="font-size:7.5px;color:#F97316;font-family:'DM Mono',monospace;">${f.scaled_grams}${f.serving_unit} · ${f.calories}kcal</div>`).join('')}</td>
            <td style="padding:6px 8px;vertical-align:top;">${day.lunch.map(f => `<div style="font-size:8.5px;color:#F1F5F2;line-height:1.4;">${f.name}</div><div style="font-size:7.5px;color:#22C55E;font-family:'DM Mono',monospace;">${f.scaled_grams}${f.serving_unit} · ${f.calories}kcal</div>`).join('')}</td>
            <td style="padding:6px 8px;vertical-align:top;">${day.snacks.map(f => `<div style="font-size:8.5px;color:#F1F5F2;line-height:1.4;">${f.name}</div><div style="font-size:7.5px;color:#3B82F6;font-family:'DM Mono',monospace;">${f.scaled_grams}${f.serving_unit} · ${f.calories}kcal</div>`).join('')}</td>
            <td style="padding:6px 8px;vertical-align:top;">${day.dinner.map(f => `<div style="font-size:8.5px;color:#F1F5F2;line-height:1.4;">${f.name}</div><div style="font-size:7.5px;color:#A855F7;font-family:'DM Mono',monospace;">${f.scaled_grams}${f.serving_unit} · ${f.calories}kcal</div>`).join('')}</td>
    ...
    <div style="position:relative;z-index:1;flex-shrink:0;margin:0 26px 6px;background:#131614;border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:11px 14px;overflow:hidden;">
      ...
      <div style="display:flex;align-items:center;gap:14px;">
        ...
        <div style="background:#1C1F1D;border-radius:12px;padding:11px 14px;text-align:center;border:1px solid rgba(255,255,255,.08);flex-shrink:0;">
          ...
          <div style="font-size:8px;color:#7A8C82;line-height:1.6;">Pick Day 1<br>Weigh portions<br>Hit ${plan.target_calories} kcal<br>Repeat 7 days</div>
        </div>
      </div>
    </div>
...
function defaultIntake(plan: MealPlan, intakeData?: any) {
  return intakeData ?? {
    weight_kg: 75, height_cm: 170, age: 28,
    gender: 'male', activity_level: 'moderately_active',
    target_weight: 70, target_calories: plan.target_calories,
  }
}

/** Pages 1–3: Health Snapshot · Macros & Roadmap · Calorie Burn Engine */
export function renderChunk1(plan: MealPlan, intakeData?: any): string {
  const intake = defaultIntake(plan, intakeData)
  return htmlDoc('CalorieIndia — Pages 1–3',
    page1(plan, intake) + page2(plan, intake) + page3(plan, intake))
}

/** Pages 4–6: 80/20 Method · Breakfast Library · Lunch Library */
export function renderChunk2(plan: MealPlan, intakeData?: any): string {
  const intake = defaultIntake(plan, intakeData)
  return htmlDoc('CalorieIndia — Pages 4–6',
    page4(plan, intake) +
    mealLibraryPage(plan, 5, 9, 'breakfast', 'Breakfast', '☀️', '#F97316', '#FB923C', 'Breakfast Foods', [
    ...
    mealLibraryPage(plan, 6, 9, 'lunch', 'Lunch', '🍱', '#22C55E', '#4ADE80', 'Lunch Foods', [
...
/** Pages 7–9: Snacks Library · Dinner Library · 7-Day Schedule */
export function renderChunk3(plan: MealPlan, intakeData?: any): string {
  return htmlDoc('CalorieIndia — Pages 7–9',
    mealLibraryPage(plan, 7, 9, 'snacks', 'Snacks', '🍎', '#3B82F6', '#60A5FA', 'Snacks Foods', [
    ...
    mealLibraryPage(plan, 8, 9, 'dinner', 'Dinner', '🌙', '#A855F7', '#C084FC', 'Dinner Foods', [
